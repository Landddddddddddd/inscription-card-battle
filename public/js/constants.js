...
export const CHANGELOG = [
  {
    version: 'v0.7.4',
    date: '2026-08-16',
    title: '对战内可折叠「键位小抄」浮窗',
    items: [
      '新增对战内键位小抄浮窗：按 <b>K</b> 或反引号 <b>`</b> 随时呼出 / 收起（也可点控制区「⌨ 键位」按钮）。浮窗读取当前生效键位（含自定义后绑定），紧凑列出战斗全部快捷键：选/放牌、移动光标、结束回合、1-4 指定列、取消、投降、求和、6 个表情。',
      '小抄浮窗与「⚙ 键位设置」同源（共用 keybinds），改键后小抄同步更新；对局结束后仍可呼出查看。玩法说明「六、键盘操作」节也补注了 K / ` 呼出小抄的提示。',
    ],
  },
  {
    version: 'v0.7.3',
    date: '2026-08-16',
    title: '玩法说明接入键位一览 · 表情音效',
    items: [
      '「玩法说明」新增「六、键盘操作（可自定义）」节：动态读取当前生效键位（含你自定义后的绑定），分「对战界面 / 组卡界面」两张表展示全部快捷键与 WASD 主轴说明，并提示输入框打字不误触、设置里按任意键重绑。',
      '表情新增专属音效（emote）：发送任意表情时播放短促上扬提示音，不再复用 click；线上对局音效仅本地播放、不影响转发。',
    ],
  },
  {
    version: 'v0.7.2',
    date: '2026-08-16',
    title: '自定义键位（WASD 为主轴）+ 表情系统 + 组卡 Shift±5 快捷',
    items: [
      '新增「⚙ 键位设置」（主菜单入口）：战斗 / 组卡两套上下文的全部动作均可重绑，点「修改」后按任意键即生效，Esc 取消本次重绑，「恢复默认键位」一键回退；改动即时存入本机浏览器（localStorage），下次自动生效。',
      '广泛应用 WASD：战斗中 A/D（含 ←/→）移动光标（未选牌移手牌、选中后移列），W 选牌 / 在光标列放牌，S/Esc 取消；组卡中 WASD 移动卡牌焦点。',
      '战斗可绑动作：移动光标、选牌/放牌、结束回合、1-4 指定列出牌、投降（F）、求和（P）、以及 6 个表情（默认 Q/E/R/T/G/H）。数字 1-9 仍作为内置快捷（未选牌直接选该张、选中后在 1-4 列打出）。',
      '新增表情系统：战斗界面 6 个表情按钮（👍😂😡🙏🔥💀），点击或按键发送，发送方一侧浮层弹出；线上 P2P 对局会实时转发给对方（房主=A / 挑战者=B）。',
      '组卡新增「选卡组快捷」：Shift + → 给焦点卡 +5、Shift + ← 给焦点卡 −5（默认绑定，可在键位设置中改）；另有 =/− 增减 1，Enter/空格 确定并开始，Esc 返回菜单；组卡界面卡牌焦点以金色描边高亮。',
    ],
  },
  {
    version: 'v0.7.1',
    date: '2026-08-16',
    title: '机制：牌库抽干不循环 · 双方出完即平局 · 新增投降与求和',
    items: [
      '卡牌出完了就是出完了：移除弃牌堆洗回牌库的循环抽牌，牌库抽干即止，不再摸牌、也没有疲劳 buff/debuff。',
      '平局判定：当双方「手牌与牌库皆空」且天平与场面连续一轮（A 回合开始快照）无变化——包括双方单位都只向天平平推、分数差永远拉不开的真·僵局——判平局。',
      '新增「投降」：认输即对方直接获胜（仅自己回合可发起，有二次确认）。',
      '新增「求和」：申请平局，每方一局仅可申请 1 次；对方可选择「接受（平局）」或「拒绝（继续）」。拒绝后对方仍可在其唯一一次机会内反申请。单机模式下 AI 会根据天平形势自动回应（落后/平局接受、领先拒绝）。',
      '无头仿真五阵营（PER=500，抽干机制下）：spread 2.9pt（≤10 达标），且对局平均回合数大幅缩短、稳定终止无死循环。',
    ],
  },
  {
    version: 'v0.7.0',
  ...
  },
  {
    version: 'v0.6.7',
    date: '2026-08-16',
    title: '每回合时限硬编码为 20 秒 · 菜单按钮加可见选中态',
    items: [
      '「每回合时限（PvP/PvE）」不再可调：移除关闭/10s/20s/30s/60s 选项按钮，统一硬编码为 20 秒（原选关闭/10s/30s/60s 的旧存档将被强制拉回 20 秒）。',
      '修复菜单中「AI 难度」「人机出牌速度」按钮没有任何可见选中态的 bug——现在选中的按钮会以金色高亮 + 描边发光显示，一眼可辨当前所选项。',
    ],
  },
  {
    version: 'v0.6.6',
    date: '2026-08-16',
    title: '组卡组支持多套保存（我的卡组）',
    items: [
      '组卡界面新增「我的卡组」管理条：可命名保存多套卡组，下拉切换读取、另存为、删除，全部存于本地（localStorage）跨局保留。',
      '「确定并开始」时会自动把当前卡组存入「我的卡组」（覆盖同名/同内容或新建），下次进组卡界面默认高亮上次的卡组。',
      '旧版「最后使用卡组」会自动迁移为「默认卡组」第一条，已有存档无缝兼容。',
    ],
  },
  {
    version: 'v0.6.5',
  ...
  },
  {
    version: 'v0.6.2',
    date: '2026-08-16',
    title: '战斗演出 · 卡面剪影 · 引导动画',
    items: [
      '战斗演出升级：召唤时从列中心冲出光能柱；单位死亡改为碎片四散的碎裂动画；血肉献祭时播放红色血光上涌的血祭特效（与献祭音效对齐）。',
      '打击反馈增强：连击（double_strike）触发金色「双击!」爆点与屏幕强震动；致死（death_touch）触发紫色「即死!」；尖刺反伤飘「反伤」提示；天平重击显示 ×2。',
      '卡面独特剪影：每张卡在共用基础造型之上，依据印记（飞行加翼、连击/高攻加角、尖刺加刺、厚甲加甲片、致死红眼、毒触毒滴、再生光环）与费用（高费加华丽尾）叠加差异化轮廓，并由卡牌 ID 决定轻微姿态倾斜，同造型的卡也各不相同。',
      '新手引导动画化：教程的「出牌」步指向手牌区、「结束回合」步指向对应按钮，目标元素金色脉冲高亮 + 动态箭头浮动引导；交互步遮罩背景更淡，保证目标清晰可见。',
    ],
  },
  {
    version: 'v0.6.1',
    ...
    title: '观感打磨与手感修正',
    items: [
      '修复手牌 Bug：移除了「所有手牌每次重渲染都重弹入场动画」的全局规则，改为仅新抽到的牌才播入场动画。此前每次选牌 / 切换回合整手牌都会反复跳一下，观感廉价。',
      '棋盘上敌方单位不再显示手型光标（不能点），仅己方可操作的手牌 / 献祭目标保持手型。',
      '轮到玩家出牌时，回合标签（轮到：xxx）改为金色呼吸高亮，强化「该你出牌了」的提示。',
      '新增非战斗加血反馈：regen 等导致的生命提升会在单位上方飘出绿色「+N♥」，与挨打时的红色「-N」飘字呼应。',
    ],
  },
  {
    version: 'v0.6.0',
    date: '2026-08-15',
    title: '键盘交互（战斗界面全键盘操作）',
    items: [
      '战斗界面新增完整键盘操作：数字 1-9 选中手牌；选中牌后按 1-4 或 Q/W/E/R 在对应列出牌（空列）或献祭（己方单位列）；方向键 ←→ 移动手牌 / 列光标；空格 / Enter 结束回合（无选牌时）或在列光标打出（有选牌时）；Esc 取消选牌 / 献祭。',
      '手牌左上角显示键盘序号角标，战场列号在选中牌时高亮为「出牌（金）/ 献祭（红）」目标，并新增底部键盘操作提示条，键位一目了然。',
      '鼠标点击操作完全保留，键盘与鼠标可任意混用。',
    ],
  },
  {
    version: 'v0.5.3',
    ...
    title: '在线联机 · 健壮性与可达性',
    items: [
      '线上联机（P2P）新增「信令自动降级」：若 PeerJS 信令服务不可用，自动切换到「邀请码」模式（纯 WebRTC + 公共 STUN，无需任何服务器），联网成功率大幅提升。',
      '本地联机（运行 node server.js 的权威转发）新增网络异常兜底：服务器未运行 / 房间无效时，给出清晰指引而非静默失败。',
      '主菜单新增「联机模式说明 / 服务器状态探测」：非本机部署时提示线上联机为点对点；本机部署时实时检测 server.js 是否在运行。',
    ],
  },
  {
    version: 'v0.5.2',
